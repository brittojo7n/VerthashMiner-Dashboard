...
- **Miner Process Status:** `miner.running` (boolean), `pid`, `startedAt`, `exitCode`, `signal`, `lastLine`, `lastError`, and parsed wallet address `miner.wallet` (derived from the `-u`/`--user` flag up to the first worker dot `.`).
