...
| **`server/utils/config.js`**    | Loads and clamps settings from `.env` (overridden by system environment variables), parses flags in `MINER_ARGS`, injects `--protocol-dump`, and extracts wallet addresses and device mappings. | Environment variables / `.env` file                             |
| **`server/utils/constants.js`** | Single source of truth for status enums, log classifications, and hard runtime caps.                                                                                                            | Internal module exports                                         |
| **`server/utils/state.js`**     | Holds the central in-memory state object, maintains a 25-entry ring log buffer, outputs structured snapshots, and merges GPU metrics with hashrates via PCI IDs.                                | In-memory reference                                             |
| **`server/utils/timers.js`**    | Wraps `setTimeout` and `setInterval` with unreferenced (`unref()`) timers to prevent background tasks from keeping the event loop hostage during exits.                                         | Internal utilities                                              |
...
| **`server/http/bundle.js`**    | In-memory asset bundler that transforms frontend ESM source from `web/services/app` into an IIFE bundle when the server starts.                                                                     | In-memory build                                                 |
...
- **0 Active SSE Clients:** The miner runs untouched in the background, but standard output line tokenization, chunk buffering, and `nvidia-smi` subprocess polling are completely paused.
...
`server/utils/config.js` reads a dotenv-style file (default `.env` next to `main.js`, or the path in `ENV_FILE`) and copies only keys that are not already present on `process.env`. Operating-system environment variables therefore always win.
...
| -------- | -------------------- | ------------------- | ------------------------------------------ | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **GET**  | `/health`            | None                | None                                       | None         | Returns JSON health payload `{ status: "ok", miner: { running, pid, status }, gpu: { count/error }, sse: { clients }, uptime }`.                                                                          |
...
| **GET**  | `/events`            | Cookie (if enabled) | None                                       | 3 req / 2s   | Upgrades to SSE. Emits `: stream established`, then a complete `event: stats` snapshot; subsequent stats frames are throttled to 50ms, single-pass serialized, and carry only delta logs newer than that client’s sequence. Heartbeat comments (`: hb`) every 15s. Max 4 concurrent connections; overflow receives `event: rejected`. |
...
- **Miner Process Status:** `miner.running` (boolean), `pid`, `startedAt`, `exitCode`, `signal`, `lastLine`, `lastError`, sanitized user string `miner.user`, parsed wallet address `miner.wallet`, and optional worker name `miner.worker`. All three identity fields come from the `-u` / `--user` argument. Text before the first `.` is the wallet; non-empty text after it is the worker. A bare address or a trailing dot with no worker (`vtc1q….`) is rewritten to the wallet only before the miner is spawned, and the Worker field is not shown. A value such as `vtc1q….Tuf_A15` is passed through as `wallet.worker`.
...
At listen time `buildAssets` reads `web/`, rewrites the HTML module tag to `/app.js`, and runs the import/export transformer in `server/http/bundle.js` over the graph starting at `services/app`. The result is a single IIFE with numeric module ids. Internal source paths such as `/web/*` and `/js/*` are never registered.
