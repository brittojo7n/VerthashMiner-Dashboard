...
export const stripLogPrefix = (text) =>
  String(text).replace(/^\[[^\]]+\]\s*\w+\s*/, "");
const tempStatus = (t, levels) =>
  t == null
    ? null
    : t >= levels.hot
    ? "danger"
...
export const LIVE_STATUS = new Set([
  "MINING",
  "CONNECTED",
  "WAITING",
  "DISCONNECTED",
...
function effectiveStatus(snapshot, pendingStatus) {
  ...
  return !snapshot.miner.running && LIVE_STATUS.has(status)
    ? "STOPPED"
    : status;
}
...
export function presentSnapshot(snapshot, options = {}) {
  ...
  return {
    ...
    ratio:
      snapshot.acceptedRatio == null
        ? DASH
        : `${num(snapshot.acceptedRatio, 2)}%`,
...
export function presentGpu(gpu, opts = {}) {
  ...
  const eff =
    gpu.hashrate > 0 && gpu.powerW > 0 ? gpu.hashrate / gpu.powerW : null;
  const util =
    gpu.utilizationPct == null
      ? 0
      : Math.max(0, Math.min(100, gpu.utilizationPct));
  return {
    name: `GPU ${gpu.index} \u2022 ${gpu.name || "Unknown"}`,
    pstate: gpu.pstate || DASH,
    temp:
      gpu.temperatureC != null ? `${num(gpu.temperatureC, 0)}\u00b0C` : DASH,
