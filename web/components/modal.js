...
export function createModal() {
  ...
  const close = () => {
    ...
    const cb = onClose; dismissable = false; onClose = null;
    dialog.textContent = "";
  ...
  const onKey = (e) => {
    if (e.key === "Escape") { if (dismissable) close(); return; }
    if (e.key !== "Tab") return;
  ...
  backdrop.addEventListener("click", (e) => { if (dismissable && e.target === backdrop) close(); });
  ...
  return { open, close, dialog, backdrop, get isOpen() { return backdrop.classList.contains("show"); } };
}
