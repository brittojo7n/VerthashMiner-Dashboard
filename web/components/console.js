...
function cacheUsable() {
  ...
  try {
    ...
    console.error("[dashboard] storage probe failed:", err.message);
    storageUsable = false;
  }
...
function isReloadNavigation() {
  try {
    ...
    console.error("[dashboard] navigation check failed:", err.message);
  }
  return false;
}
function cacheClear() {
  try {
    ...
    console.error("[dashboard] cache clear failed:", err.message);
  }
}
function sanitize(entries) {
  ...
  for (const entry of entries) {
    if (!entry || typeof entry.id !== "number" || !Number.isFinite(entry.id))
      continue;
...
function cacheLoad() {
  ...
  try {
    ...
    console.error("[dashboard] cache load failed:", err.message);
    cacheClear();
    return [];
  }
}
function cacheSave(entries) {
  ...
  try {
    ...
    storageBroken = true;
    cacheClear();
    console.error("[dashboard] cache save failed:", err.message);
  }
}
const RULES = [
...
function highlight(raw) {
  ...
  for (let i = 0; i < RULES.length; i++)
    painted = painted.replace(
      RULES[i][0],
      `<span class="${RULES[i][1]}">$1</span>`,
    );
  return painted;
}
const EMPTY_HTML =
  '<div class="log-empty"><span class="log-prompt">&gt;</span><span class="log-text">VerthashMiner console initialized. Waiting for miner output...</span><span class="term-cursor">_</span></div>';
export function createConsole({
  ...
  terminal.addEventListener(
    ...
    () => {
      const atBottom =
        terminal.scrollHeight - terminal.scrollTop - terminal.clientHeight < 25;
  ...
  return {
    ...
    render(entries, meta = {}) {
      try {
        const serverCount = Number.isFinite(meta.count)
          ? meta.count
          : (entries || []).length;
        ...
        let added = 0;
        for (const entry of entries) {
          if (!entry || typeof entry.id !== "number" || entry.id <= maxId)
            continue;
          frag.appendChild(buildRow(entry));
          history.push({
            id: entry.id,
        ...
        if (typeof console !== "undefined" && console.error)
          console.error("[dashboard] console render failed", err);
      }
    },
  };
}
