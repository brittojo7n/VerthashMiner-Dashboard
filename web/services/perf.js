...
function createPerfGate(env) {
  ...
  function setMode(mode, reason) {
    self.reason = reason; self.mode = mode;
    if (root && root.classList) {
      if (mode === "fx") root.classList.add("fx"); else root.classList.remove("fx");
    }
  }
  function lockLite(reason) { self.locked = true; setMode("lite", reason); }
  function probe() {
    if (self.locked) return false;
    if ((navigator.hardwareConcurrency || 4) < 4) setMode("lite", "weak device");
    else setMode("fx", "probe passed");
    return true;
  }
  function start() {
