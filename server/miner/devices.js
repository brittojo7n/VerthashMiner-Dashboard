  ...
  return function handleChunk(chunk) {
    if (forward) {
      try {
        ...
        console.error("[dashboard] forward chunk failed:", err.message);
      }
    }
    ...
    if (cut === -1) {
      if (buffer.length > LIMITS.STREAM_BUFFER_BYTES)
        buffer = buffer.slice(-LIMITS.STREAM_BUFFER_BYTES);
      return;
    }
    ...
    if (typeof onFlush === "function") {
      try {
        ...
        console.error("[dashboard] stream onFlush failed:", err.message);
      }
    }
  };
}
